...
UBOND_PATCH8_B64="$(base64 < "${PATCHES_DIR}/ubond_cpu_pacing_busywait.patch" | tr -d '\n')"
...
ssh -p "${RPi_SSH_PORT}" "${RPi_USER}@${RPi_IP}" \
    ...
    UBOND_PATCH8_B64="${UBOND_PATCH8_B64}" \
    bash <<'REMOTE_SCRIPT'
set -euo pipefail
...
if command -v ubond &>/dev/null; then
    ...
    # REQ-NET-27: fix H1+H2 — data_seq compartido + dedup return contract
    printf '%s' "${UBOND_PATCH3_B64}" | base64 -d > /tmp/ubond_replicate_dedup_fix.patch
    if ! patch -p1 -N --reject-file=- < /tmp/ubond_replicate_dedup_fix.patch 2>&1 | head -10; then
    ...
    # REQ-NET-46: elimina busy-wait CPU 99% (pacing por lotes). Tres spinners
    # que forzaban select(timeout=0) en libev: do_send rama "too soon"
    # (ev_check->ev_timer + envío en lote), recalc_weight (floor 5ms al
    # send_timer.repeat, antes ~8kHz) y reorder_drain_check (ev_check->ev_timer).
    # Bug de diseño upstream. Validado: CPU 99%->oscila con carga, 0% pérdida.
    printf '%s' "${UBOND_PATCH8_B64}" | base64 -d > /tmp/ubond_cpu_pacing_busywait.patch
    if ! patch -p1 -N --reject-file=- < /tmp/ubond_cpu_pacing_busywait.patch 2>&1 | head -10; then
        echo "  [RPi] (patch cpu_pacing_busywait ya aplicado o no aplicable; continuando)"
    fi
    rm -f /tmp/ubond_cpu_pacing_busywait.patch
