#!/bin/sh
# Validates ave-vpc.REQ-NET-01: iPhone USB tethering reconocido por macOS.
# shellcheck disable=SC1091
. "$(dirname "$0")/_lib_junit.sh"
junit_init "REQ-NET-01_iphone_tethering"
...
if [ -n "${IP}" ]; then
    junit_pass "iphone_${IFACE}_ip_${IP}"
else
    junit_skip "iphone_no_ip" "${IFACE} sin IP — Personal Hotspot no activo"
fi
junit_finalize
