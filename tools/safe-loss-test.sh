...
MAX_SECONDS=75          # timeout duro global
...
# === 3. medir ===
log "--- ESTABILIZANDO 15s antes de medir ---"; sleep 15
log "--- pérdida ventana larga (20 pings tras warmup) ---"; timeout 30 ping -c15 "${TUN_VPS}" 2>&1 | grep -E "loss|min/avg" | tee -a "${RESULT}"
...
log "(iperf omitido en test de pérdida)"
